...
describe('ReviewService', () => {
    ...
    const reviews: Review[] = [
        {review_id: 1, manga_id: 1, content: "", created_at: new Date(), user_id: 1, rating: 1}
    ]
    ...
    beforeEach(async () => {
        const moduleRef = await Test.createTestingModule({
            providers: [
                ReviewService,
                MangasService,
                { provide: getRepositoryToken(Review), useValue: reviewsRepositoryMock },
    ...
    it("should return reviews for manga", async () => {
        jest.spyOn(reviewsRepository, "createQueryBuilder").mockImplementation(() => {
            return {
    ...
    it("should add review", async () => {
        ...
        jest.spyOn(mangasService, "getOne").mockResolvedValue({manga_id: 2} as Manga)
