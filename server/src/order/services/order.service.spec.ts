...
describe('OrdersService', () => {
    ...
    it("should create order", async () => {
        ...
        jest.spyOn(paymentService, "getPaymentMethodID").mockResolvedValue({ payment_method_id: 1 })
        jest.spyOn(ordersRepository, "save").mockImplementation((order) => {
            return Promise.resolve({...order} as Order)
        })

        expect(await ordersService.createOrder(request, dto)).toEqual(JSON.stringify("Order created"))
        expect(ordersRepository.save).toHaveBeenCalledWith(
            {
                payment_method_id: 1, total_price: 10, user_id: 1,
                order_status: OrderStatus.PENDING, order_date: expect.any(Date),
    ...
    it("should return order statues", () => {
        expect(ordersService.getOrderStatuses())
            .toEqual(Object.keys(OrderStatus).map(key => OrderStatus[key]))
    })
